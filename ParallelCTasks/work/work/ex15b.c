...
#include "myio.h"
 ...
 double *tabled_sin1, *tabled_sin2;
...
double opt_sin(int k, int i, int n, double *tabled_sin) {
  int loc_m, sign;
  loc_m = k*i % (n/2);
  switch ( (2*k*i/n) % 4) {
  case 0:
...
//
  ...
  tabled_sin1 = (double*)(malloc(sizeof(double)*(n1/2+1)));
  tabled_sin2 = (double*)(malloc(sizeof(double)*(n2/2+1)));
  ...
  if (mp==0) {
    for (i1=0; i1<=n1/2; ++i1) tabled_sin1[i1] = dsin(pi*i1/n1);
    for (i2=0; i2<=n2/2; ++i2) tabled_sin2[i2] = dsin(pi*i2/n2);
  }
  
  if (np>1) {
    MPI_Bcast(tabled_sin1,n1/2+1,MPI_DOUBLE,0,MPI_COMM_WORLD);
    MPI_Bcast(tabled_sin2,n2/2+1,MPI_DOUBLE,0,MPI_COMM_WORLD);
  }
    ...
    for (k2=0; k2<=n2; k2++) {
      for (k1=0; k1<=n1; k1++) {
        ...
        for (i2=1; i2<n2; i2++) {
          for (i1=1; i1<n1; i1++) {
            m = n1p * i2 + i1;
            s1 = opt_sin(k1, i1, n1, tabled_sin1);
            s2 = opt_sin(k2, i2, n2, tabled_sin2);
            s0 += yy[m]*s1*s2;
          }
        }
