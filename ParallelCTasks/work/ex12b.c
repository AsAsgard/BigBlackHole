...
double f1(double u) {		// уже с учётом k(u)=1, k'(u)=0
  return q1(u)*u+q(u)-k(u)*u3(u);
}
...
// Iterations:
  ...
  do {
    ...
    if (mp==0) {	// ГУ слева
      ...
      aa[0] = 0.5 * (s0 + s1);
      bb[0] = 0.5 * (s0 + s2);
      cc[0] = hx2 * q(y0[0]) + aa[0] + bb[0] + thetta * (hx2 * q1(y0[0]) * y0[0] - hx2 * f1(y0[0]));
      ff[0] = hx2 * f(y0[0]) + thetta * (hx2 * q1(y0[0]) * y0[0] - hx2 * f1(y0[0])) * y0[0];
    }

    for (i=1; i<ncm; i++) {	// внутренние расчеты
      s0 = k(y0[i]); s1 = k(y0[i-1]); s2 = k(y0[i+1]);
      aa[i] = 0.5 * (s0 + s1);
      bb[i] = 0.5 * (s0 + s2);
      cc[i] = hx2 * q(y0[i]) + aa[i] + bb[i] + thetta * (hx2 * q1(y0[i]) * y0[i] - hx2 * f1(y0[i]));
      ff[i] = hx2 * f(y0[i]) + thetta * (hx2 * q1(y0[i]) * y0[i] - hx2 * f1(y0[i])) * y0[i];
    }

    if (mp==np-1) {		//ГУ справа
      ...
      aa[ncm] = 0.5 * (s0 + s1);
      bb[ncm] = 0.5 * (s0 + s2);
      cc[ncm] = hx2 * q(y0[ncm]) + aa[ncm] + bb[ncm] + thetta * (hx2 * q1(y0[ncm]) * y0[ncm] - hx2 * f1(y0[ncm]));
      ff[ncm] = hx2 * f(y0[ncm]) + thetta * (hx2 * q1(y0[ncm]) * y0[ncm] - hx2 * f1(y0[ncm])) * y0[ncm];
    }

    rka = 0.0;

    for (i=0; i<nc; i++) {		// подготовка к переходу на новый слой?
      s0 = y0[i];
    ...
    if (lp>0)			// вывод промежуточных расчетов
      for (i=0; i<nc; i++)
        fprintf(Fo,"i=%8d a=%12le b=%12le c=%12le f=%12le\n",
          (i1+i),aa[i],bb[i],cc[i],ff[i]);

    if (np<2) ier = prog_right(nc,aa,bb,cc,ff,al,y1);		// весь метод прогонки это эти 2 строки
